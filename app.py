...
def irr_bisection(cashflows: list[float]) -> float | None:
    ...
    if not (np.any(np.array(cashflows) > 0) and np.any(np.array(cashflows) < 0)):
        return None
            ...
            try:
                ...
                if missing:
                    st.error(
                        ...
                        + ". Mẫu cột đúng: " + ", ".join(required)
                    )
                else:
                    df_up = df_up[required].copy()
                    df_up["Ngày"] = pd.to_datetime(df_up["Ngày"], errors="coerce")
                    df_up["Số tiền"] = pd.to_numeric(
                        df_up["Số tiền"], errors="coerce"
                    )
                    df_up = df_up.dropna(subset=["Ngày", "Số tiền"])
                    df_up = df_up[df_up["Số tiền"] >= 0]
                    st.session_state.cashflows = df_up.reset_index(drop=True)
                    st.success(f"Đã nạp {len(df_up):,} giao dịch.")
            except Exception as exc:
